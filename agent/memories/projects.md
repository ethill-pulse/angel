...
## Aug 4 heartbeat — 5-week gap notice

**No journaling occurred between 2026-06-30 and 2026-08-04.** This heartbeat reconstructs that period from: GitHub PR history (665 merged PRs across pulse+polaris), the Milestone Summary / Exec Summary tracker exports, Gemini meeting-note emails (Digital Dev Sync, Crypto EOW, Parataxis Daily catchup, CAAS MVP), and Eric's own notes (`20260714_halo_presentation.md`, `20260721_team_priorities.md`). Calendar access was denied this session (known VS Code-terminal TCC issue — see [[feedback fix in journal]], re-run from Terminal.app to restore). No Notion search performed — the Notion MCP connection needs re-authentication.

### ⭐ Headline: the July-15 low-touch client (Parataxis) launched, and polaris was absorbed into pulse

- **Parataxis go-live — COMPLETE.** The "July 15 client" has a name: **Parataxis**. Milestone tracker: *"Low Touch Principal Execution - Parataxis Go Live"* = **Complete** ("tactical solution to support Parataxi go live is completed"). Daily "Parataxis - Daily catchup" meetings ran Jul 7–20 (account migration off the test algo, obligation-netting design, BTC buyback to clear a short, VPN/access issues for ops). Treat the July-15 Phase-1-direct-API push (Eric's FIX/WS codegen, Matt Gow's security review) as **done** — the security/access-provisioning action item that was Eric's is resolved by the fact of launch (no explicit "approved" email seen, but the client is live).
- **`polaris` repo has been merged into `pulse`.** Confirmed via PRs and the Aug 4 Digital Dev Sync ("Polaris codebases now exist within the Pulse architecture"). Erick Arce (earce-clearstreet) drove it Jul 27–30, culminating in six huge single-day PRs Jul 30 including **#3210 "Port risk quoting and execution" (+68274/-15)** — the largest PR in the whole dataset. The `pulseprime/polaris` repo went cold after Jul 30 (its last PR: #1137). Ömer (litityum) migrated the polaris-only Claude skills and restored `libs/polaris/CLAUDE.md` inside pulse; EmreEkici ported the README. **Structural implication: `polaris` is no longer a separate repo** — algo/quoting/OTC-hedging code (previously ant0wn/litityum/EmreEkici's domain) now lives at `libs/polaris` inside the pulse monorepo. Top-level `CLAUDE.md`'s repo layout description is now stale on this point. A same-week, related type-hardening pass: **#3257 "Move all decimal to consistent try_from for floats" (+4732/-2750, 211 files)**, Erick, Jul 31.
- **HT OTC Options Test Trade — COMPLETE** ("Completed F2B testing," forecast was Jul 24, done by Jul 31 per Crypto EOW). **HT OTC Options Live [Client Ready] flipped Red→Green**, forecast **Aug 14**. 12 scenarios spec'ed for testing. Remaining gates are non-eng: legal/account-opening still blocking the next round of production testing; **Risk has no RMS-UI resource until the Aug 17 sprint** (may delay client experience); FACT manual instrument setup; KOR reporting needs a CS Digital LEI; CSD↔Cayman hedge leg (Cayman→RenGen→Deribit) still missing.
- **CFTC Swap Dealer Consultant decision reversed**: tracker flipped Red→**Complete** — "decision made to staff internally and fund the roles for risk/finance/operations" (the Potomac consultant plan is out). Swap Dealer *Application* itself still Red, blocked on Risk hire (none yet) and Finance hire (expected Aug 2026).
- **🆕 Pre-IPO Swap Trade** — new initiative, not previously tracked. Robert Rutherford (Bob) to align with John DiBacco on prioritization; paused mid-July then compliance cleared it to proceed (per Jul 31 Crypto EOW). **Portfolio Swap / HT Perp / LT Perp / Spreader-routing-perps are all now Paused/De-prioritized** in favor of Pre-IPO Swap work — a real reprioritization, not just a slip.
- **Stablecoin Semi-Automated — regressed Amber** (was complete-ish), forecast pushed **+1 week to Aug 7**: bugs from the first F2B trade fixed, but **the Sweden-based counterparty team's holiday limited execution capacity**. Automated flow still Not Started, gated on a funding/reserve-calc requirement and an EY sign-off review with legal.
- **Loan-booking incident (Jul 31, ops not eng):** incorrect loan booking records (120.5 BTC / $5M) caused month-end reporting discrepancies — Rita/Anoop/Ritik manually journaling corrections; Lily Chen to find out from David why the entry was booked wrong.

### Team activity, Jun 30 – Aug 4 (PR-derived — full detail in `pr_activity.md`)

- **Eric (59 PRs)**: two disciplined phased initiatives — FIX session persistence/recovery (phases 7.4→13.9, Jul 6–15) rolling directly into an order-status protocol initiative (phases 0→5, Jul 15–30) — plus the `QueuedDataStoreAppender` library. This is the eng-side of the "disk-backed queue for order-status recovery" decision reconfirmed at today's (Aug 4) Dev Sync.
- **Chris (52 PRs)**: two long-running series spanning the whole window and still in flight as of Aug 3 — "Risk Cost Basis Model" (1.1→9.1) and "Unrealized PnL" (1.1→3.3). Today's Dev Sync gave him fresh action items: verify prod-test feasibility with the loan-tech team, set up test accounts, build a new options-cancel mechanism, and fix build scripts causing disk-space exhaustion.
- **Amit (39 PRs)**: steady, no drift — GTC/resting-order execution correctness (partial fills, re-slicing, amend races, crash-guards).
- **Ömer/litityum (181 PRs, top volume)**: full Kraken spot venue build, a margin/`collateral_mode` reporting overhaul rolled out across nearly every venue, plus heavy involvement in the polaris migration closeout.
- **Talgat (55 PRs)**: Jul 7–21 rapid Trade-Engine-Bots iteration, then Jul 26–30 shifted to Trade-Engine persistence (Redb, QuoteCache durability). **Went quiet Jul 30 → Aug 4** — worth a check-in (OOO, reassigned, or between milestones — unclear).
- **Estiven (54 PRs)**: Jul 3–9 burst of ~15 Admin UI widgets in a day; Jul 16–26 pivoted to session-state/halt controls + FIX audit trail. **Gap Jul 27–Aug 2**, then reappeared Aug 3 with a **new "loans-widget" thread** (#3310) — first appearance, likely a new initiative worth tracking.
- **Atakan (3 PRs only)** — lowest volume of any report across 5 weeks. Today's Dev Sync has him owning Haruko/Talos reconciliation dev work (action item: notify team when it's deployed to dev), so he's not idle, just not shipping via PR yet. One of his PRs (#2917, testnet integration) was reverted by Eric (#2927) and later **relanded by Aksel** (#3092) under the same title — worth understanding whether that's a permanent ownership handoff.
- **Aksel (8 PRs)**: mostly bugfix/hardening early, then reconciliation work late (Jul 23/29/30) — overlaps with Atakan's recon assignment above.
- **Emre (39 PRs)**: per-venue leverage/margin_mode population, then OTC hedge-fill-debit tracking in polaris, then porting his own polaris pieces into pulse post-migration.
- **Matt Gow (4 PRs)**: light but steady infra footprint; also ran point on the Aug 4 Dev Sync's new "thread capture" debugging tool and deployment-window announcements (**prod deploys now restricted to after 11pm** to protect FIX traffic).
- **🆕 SelmanB (6 PRs, Jul 21–31)** — **unrecognized handle**, doing venue-integration work (Coinbase, a new `Venue::Ibkr` skeleton, Coinbase National, Bitso) similar in shape to litityum's. Not on the known team/consultant list — needs identification (new hire? RenGen consultant?).
- **Anton (34 PRs, almost all polaris)**: OTC/FX-translation and hedge-capacity work continued throughout; filed his *first-ever pulse-repo PRs* on Aug 4, consistent with the post-migration cutover.

### Other flags carried forward / still open
- **Org leveling** (HR title/benchmark alignment, from Jun 30 Eric/Brian 1:1) — no visible follow-through yet in this period's material; still watch for it.
- **Position-recovery system end-state** — explicitly unresolved per today's Dev Sync ("Plan Recovery: determine the end state and ideal solution" — open action item for the group).
- **CaaS** (Rama/Ankit Singh, not Eric's team): "CAAS MVP" project plan kicked off Jul 23 with new names — **Zack Yu**, **Ram Kollengode Kalyanakrishnan**, **Christie** (compliance), **Lisa Yen** (SSI/PM), **Mammud**. 1:1 client-account:BitGo-wallet model confirmed; Studio integration in scope. Active MVP tracker: Amber, forecast **Aug 28**, "very aggressive timeline." **Eric has an action item from today's Dev Sync: update the team on the status of the Custody-as-a-Service meeting Thursday (Aug 6)** — this is a distinct, real meeting per the Dev Sync notes, separate from the Tuesday-recurring "CAAS MVP" project-plan sync; it doesn't appear on Eric's own calendar (he's relaying status, not attending).
  - **Aug 4 "CAAS MVP - review/update project plan" meeting (Gemini notes)**: account-model structural decision finalized — **new account subtype** (not a separate account type) for CaaS clients, aligned with stakeholder approvals. **Retail KYC confirmed reusable** (no re-verification needed); **institutional-client requirements split into their own workstream** (more complex). **Resource constraints on wallet creation are now flagged as a real risk to the August timeline** — team members asked to populate tentative task dates for better tracking. New names surfaced: **Cindy Guo** (scheduling CSSE wallet-ownership meeting + wallet breakout session with Lisa Yen), **Shalomi** (likely the same **Shlomi Avivi** already tracked under BitGo VM/security — OAuth/MFA design being shared with him for feedback, and he's syncing with Lisa Yen on wallet security/control risk-review minimums).
- **HT Spot P1** — now **Green**, forecast **Aug 14**; blocker is BK-vs-BitGo recon in prod. Talos→Haruko recon now in dev (progress since the Jun 30 stall). EOM Activity Report went live today (Aug 4).
- **Banking Rails**: Customers Bank Wires Green/Aug 14 (UAT in progress); **cubiX 24/7 network now COMPLETE** (Jul 17) — "payment solution rolled out."
- **Client Onboarding — COMPLETE** (Jul 3): "2 clients are being onboarded through COPS."
- **Wallet Address Mgmt MVP — COMPLETE** (May 30, live, ops trained, hand-off still required).
- **Expand to Top-100 Tokens (P2)**: new milestone, Not Started, forecast Aug 31.
- **Active platform crypto MVP (Beta, 5 users)**: forecast was **Jul 31 — already past due and still Not Started** as of the Aug 4 tracker export. Worth flagging as a probable miss.

### Historical reference (superseded by the above — kept for context)
- **Options trade-limits implementation (Chris #2440, Jun 16):** "Options deal entry: pre-trade risk gating + Sell whitelist" in `trading-ops-server`. Per-account **premium budget** (not aggregated notional) seeded from the Talos `CustomerCredit` stream; Buy deals atomic check-and-hold, decrement on book, push back to Talos; limit raised only by manual Talos change. **Sells restricted to `options_sell_whitelist`** (empty list denies all Sells), take no hold. `risk_check_mode: Disabled` bypasses. State in `RiskChecksCache` (libs/prime).
- **Reconciliation plan (HT Spot, mostly not Eric's team):** Snowflake is the prerequisite. Ritik Chandak is the recon operator (Talos sandbox+prod). This finally progressed (Talos→Haruko recon now in dev per Aug 4 Dev Sync) after stalling since late June.
- **🆕 Dev themes (June 25–30)** — full PR detail in `pr_activity.md`:
  - **⭐ Trade Engine durable persistence + recovery** (pulse) — the headline cluster, confirmed live in the Jun 30 Dev Sync ("Postgres support for session durability and data recovery"). **Eric** drove staged hardening (`fix persistence phase 1–3` #2590/#2592/#2605, `fix recovery phase 4–5` #2606/#2616); **Amit** drove the Postgres layer (`resting orders persistence postgres` #2596, in-memory cache + order-manager #2574, recover-after-restart #2608, GTC pending-placement cache #2593, e2e tests #2537). Major reliability investment in the native trade engine.
  - **⭐ Eric external-connectivity codegen** (pulse): `generate fix xml files from schemas` (#2485), `json-ws-gen-digital.py` (#2587) — the codegen behind the July-15 low-touch direct-connect path (FIX + WS/JSON). Matches the team-sync TODO "Websocket / json API."
  - **New venue: Bitget UTA (BTGE/BTGT)** (Ömer, ~20 PRs across pulse + polaris): full bring-up — refdata, mktdata, WS order entry (place/cancel/modify), dropcopy, rate-limits. Third+ new venue this quarter on the venue-integration skill scaffold.
  - **Emre RiskHeadroomService** (polaris): new risk subsystem — publish per-strategy `SidedRiskHeadroom` (#866), clamp client-order fills to headroom in the matcher (#868), wire into `otc_chain` (#869), PreTradeGate authorizes filled (not order) qty (#870). **Emre stepping up on core risk** — aligns with the peer-feedback push to get him more involved.
  - **Estiven Halo UI + RBAC**: algo-management grid RPCs (#2577, 80 files), widget/api-client/rbac wiring (#2544), researcher/operations roles (#2550), risk-widget set-limit (#2566).
  - **Talgat Trade-Engine-Bots** (new subsystem): initial setup (#2562) + DGTL quoting plumbing (#2578). **Matt** new-crate scaffolding from `infrastructure.toml` + skill (#2614/#2612, demoing Jul 2). **Erick** DMA market-data API (Binance) #2580 + polaris fill-tracker/sim work, with a same-week **revert of #857** (avg_price/notional fix → #865). No pulse reverts; all authors known.
- **🆕 Dev themes (June 24–25):**
  - **Amit TE resting-order + book-rebuild push** (pulse): apply resting-order child fills before the markup gate (#2538), prepend a BookReset so `process_book` clears-then-rebuilds (#2534), customer + LP-routing accounts on resting orders (#2520), fix L2 book never populating from full-refresh snapshots (#2519). Continues the native trade-engine matching buildout.
  - **Ömer flight-deck RPC-boundary refactor cluster** (polaris): unify per-mode RPC handlers behind an `AlgoMode` trait (#843 -994), move strategy-config validation to the RPC boundary per-config (#840), cross-config validation for unique `quoting_market` per host (#842), ESP-snapshot dedup only on confirmed emission (#838). Also pulse: Paribu switchable REST-poller vs WS dropcopy for BalanceUpdate (#2527).
  - **Anton hedge-capacity position-limit gating** (polaris): cap spot hedge capacity by min/max_position (#844 +420), validate OTC perp hedging-counterparty position limits (#855), re-walk hedge allocation on book market-data events (#854), make `bbo_divergence_skew_params` required (#853). Emre: broadcast hedge-capacity changes on the stream (#849).
  - **🆕 Aksel → Paradigm refdata** (pulse #2523 +474) — Aksel now landing Paradigm refdata, consistent with the Atakan/Aksel pivot to the Options-LT / Paradigm path.
  - Estiven csv bulk-upload updates (#2529 +1471) + webserver max_payload (#2541); Talgat TE PendingCancel (#2497); Chris settlement failures (#2530); Erick standalone venue-def publish over refdata (#2522) + reject-if-symbol-not-enabled (#2521) + simulator/venue-controller fixes (polaris #848/#846). No reverts; all known authors.
- **🆕 Paribu venue integration** (Ömer, June 16): new Turkish exchange added to pulse — dropcopy migrated to streams-v2 private WS (#2430 +1285/-1061), public market data to streams-v2 (#2428), ledger events without balance snapshot (#2444). Ömer also added a **"venue-integration skill"** (#2443 +942) — reusable AI scaffold for onboarding new venues (continues the team's AI-tooling theme alongside Matt's "alfred skills").
- **🆕 New venue: Bitkub** (Ömer, June 17): Thai exchange added to pulse — skip broker coins at refdata (#2460), parse venue error envelopes / make `result`/`pagination` optional (#2458). Second new venue this month after Paribu; both built on the venue-integration skill scaffold (extended #2481/#2482 with dropcopy execution-report tracers).
- **🆕 Estiven codegen-v2 widget migration sweep** (June 17): a dozen Halo widgets refactored onto codegen-v2 in one push — loans, rfq/order, account-manager, market, manual-booking, credit, balances, trades, markup/account-tier, role-management (#2467–#2479) + haruko loans update (#2446 +1257). Follows the #2424 ts-codegen-v2 groundwork; large systematic FE type-generation migration.
- **🆕 Talgat TE resting-order executor** (June 17): #2448 resting-order executor, #2463 accounts validation, #2464 code refactor (+437/-467). Continues native trade-engine matching buildout (his domain). Amit #2465 handle filled resting orders, #2431 GET credit-reservations endpoint.
- **🆕 Polaris feeds-architecture refactor** (Ömer, June 17–18): major reorg of feed-state handling — **FeedStateRegistry** (#790 +1012/-800) sharing controller feed-state, **FeedChange context model** (#800 +794) signaling feed-state changes, OrderGuard derives required feeds from controllers (#796), quoting controller migrated into the registry/FeedChange model (#806), shared **BboTracker** one BboCache per chain (#782), quoting-feed subscriptions tracked like other feeds (#774). Plus CLAUDE.md doc upkeep (#802/#803). Architectural change to watch.
- **🆕 Anton OTC hedge-allocation push** (polaris, June 16–18): **per-tick dynamic hedge budget allocation** (#765 +1674), renamed hedge budget→allocation (#779), **OTC discount client-quote capacity by committed hedge allocation** (#783 +1069), **harden ARB/OTC config validation + param-coverage gate** (#773 +1734), enforce max inventory by reading client position under dropcopy account (#794), removed dead `basis_adjustment_bps` knob (#807). Continues OTC client-ladder risk maturation. Also AI-tooling: documented OTC/ARB models + verified pricing examples in the quant skill (#799), codified Claude-facing repo conventions + guard hooks + review/quant skills (#798).
- **🆕 Erick telemetry + venue simulator** (June 16–18): polaris telemetry refactor (#792 +1008/-957), move throttle into risk (#778); pulse venue simulator (#2457). Pairs with the earlier flight-simulator framework — testing/observability infra.
- **🆕 Chris position/settlement fixes** (June 17): close-position indicator (#2455), fix settlement-date-in-past error (#2454).
- **🆕 Trade-engine RFQ/quote-order matching maturing** (June 12–16): Talgat quote-order matching (#2403 +585) + fills logic fix; Amit resting-order fills (#2386) + credit check incl. existing resting orders for FOK/IOC (#2370). On polaris, Anton built out OTC execution risk: **BBO-divergence skew for OTC client ladder** (#770 +665), **last-look tolerance for RFQ FOK** (#767), retain RFQ price improvement at quote-bounded limit (#743). This is the Options-LT / RFQ path getting built end-to-end.
- **🆕 Flight simulator framework** (Erick polaris #769, June 16): scaffold for deterministic replay/backtesting of strategies (pairs with Anton's earlier flight-replay determinism quantization work). New testing infra to watch.
- **🆕 Estiven TS codegen-v2** (#2424 +1101, 17 files, June 13) + Halo widget WS overhaul (market widget #2433 +807, credit-risk widget #2441, order-forms reconnect retries #2436). Frontend type-generation + real-time widget refresh.
- **🆕 Chris credit/risk endpoints** (June 12–16): credit endpoint (#2412 +403), options risk (#2440 +658), block missing subaccount (#2435), host rewrite (#2408, 29 files).
- **⚠️ "Big scary book change" REVERTED** (Emre #2414, June 11, +4182/-4018, 91 files) — large book-handling change backed out. No follow-up context captured; watch for re-land.
...
- *(Stablecoin / HT Spot P1 / Cross-Entity Margin / CaaS / Portfolio Swap / Banking Rails / Client Onboarding status — see the current "Key flags" section at the top of this file; June-18 versions removed Jun 23 as superseded.)*
- **CaaS reference detail (not Eric's team):** two onboarding scenarios — existing-LLC (reuse KYC) vs net-new (full LLC + packet to BitGo). Payment API for cash-settled trades ready, pending E2E test. BitGo integration agreement returned with comments (Scott Gutmanstein + Christy Moccia). Chain Analysis contract extended for transaction monitoring. SSI Wallet Address Mgmt MVP COMPLETE (live, ops trained).
...
- **⚠️ CRITICAL: Haruko pricing model sign-off** — still the single gate for OTC options go-live. Ricky must request model docs from Haruko; Kwan team must validate. This blocks Studio real-time display and margin methodology. Options HT Live = **Red/May 30**.
- **✅ LT Options architecture decision (May 13)**: RFQ automation will go through Polaris (not trade engine) from day 1. Key reasons: same central risk book as spot/perps; polaris passes RFQ to **Paradigm** (institutional crypto options RFQ platform) rather than hitting exchange directly; natural gateway to hedging and multi-strategy routing. Eric + Anton agreed.
...
- **7-Day Rollover ✅**: Fully resolved. Rollover timing = 9:30am daily. Weekend ops via async Slack. No open eng items.
- **OTC Options — entity confirmed, architecture locked (as of May 7 Mini-Workshop)**:
  - Entity = CS Derivatives (custodial accounts). Separate accounts for swaps vs options.
  - ENA/Expiry: 30-min TWAP 4s sampling (Pulse publishes). FACT marks inactive post-expiry. BK CDC-triggered.
  - Instrument ID must include expiry **time** (not just date/strike) — business confirmation still needed (impacts hedging).
  - Buying power architecture: Haruko → service → Pulse → Talos. Talos CustomerCredit now optional (#2103). Amit + Chris own this integration.
  - Haruko margin confusion on long calls: likely because options are on margin accounts (not cash). Amit to clarify.
  - Haruko↔risk portal: Ani pulls EOD portfolio margins from Haruko API. Intraday = Ani+Ricky to define (LTV, PnL vs equity). Alerts preferred in Studio.
  - CFTC reporting: Kor is the system. BK/Pulse/Haruko→Kor data flow still needs owners.
  - Action items still open: Amit to clarify margin model with business + provide API endpoints to Ani; Lily to schedule Bob/Haruko review; Christian to follow up with Atul on model validation; team to confirm expiry-time-in-instrument-ID with business.
- **Options Pricing Engine (Talgat)**: Mark-Pricer active dev. Options added to instrument puller (Chris #2102). Talgat's CustomerCredit optional change (#2103) enables options pre-trade path.
...
- **Stablecoin→USD semi-automated**: Tracker = **Green/May 30** (improved from Amber — BK dev actively in progress). Finance/reserve/capital questions still pending Julian + Brian Oliveira. Positive signal.
- **High Touch Spot P1 (recons, confirms, PnL)**: Tracker now shows Amber with May 29→Jun 12 slip. Layoff impacted people working on Snowflake; 1-2 week delay. Automation of recons, confirms, PnL not started.
- **Kevin Stevens laid off May 4**: Cash-settled ownership vacuum resolved. Amit Kirdatt is owning CaaS, reporting to Rama (BK). Eric's team is NOT responsible for custody integrations (BitGo, CaaS). Clean split: Eric's team = execution + market data; Rama/Amit = custody/CaaS.
- **CAST/CaaS (CAST)**: Amber/Jun 30. Amit Kirdatt owns this, reporting to Rama. Not Eric's team's problem.
- **Cross-Entity Margin test trade**: Tracker = Red/May 15. Was Green — slipped. Still awaiting Julian to update Digital MLA before test trade can happen.
- **Top-50 Tokens**: Tracker = Amber/May 15. Diligence complete, handover to tech. Bob/Suley decision needed on pricing source. Talos/FACT integration is BAU.
- **CFTC Swap Dealer consultant**: Red. Potomac proposal in legal/compliance review. Christy + Bob decision pending. Target start now unclear (was May 8).
- **Tracker (May 8 update — latest)**: HT Spot = Complete; HT Spot P1 = Amber/Jun 12 (layoff impact); LT Spot = Green/May 15 (on track per exec summary, milestone shows May 22); Top-50 Tokens = Amber/May 15 (pricing source decision pending Bob/Suley); Options HT Test Trade = Amber/May 22; **Options HT Live = Red/May 30** (Haruko risk sign-off delay); **Options LT = Red/Aug 31** (big slip — previously Jun 1); Portfolio Swap = Amber/Jun 30; HT Perp = Not Started (5/15 NOT achievable, Sherby dependency); CFTC consultant = Red; Loan/Borrow = Green/May 31; **Stablecoin = Green/May 30** (improved); Cross-entity test trade = Red/May 15 (Julian MLA); CAST = Amber/Jun 30.
- **Jira epics overdue**: DIG-4 (Eric + Erick Arce) and DIG-6 (Eric + Talgat) both flagged overdue by Jira automation (May 11). Eric should update dates or close.
- **KnowBe4 security training**: ✅ Completed May 18, 2026.
- **Eric's upcoming PTO**: May 29 off (out of town for wedding). Jul 27–31 family summer vacation (full week). Both approved.
- **Peer review + monthly 1:1s**: Eric announced May 7. Peer questionnaire end of May → 1:1s start June 2026.
- **Venue code deprecation**: Emre flagged Erick said venue codes are being deprecated. Eric pushed back. Need sync with Erick to resolve the broader pattern question.
- **Eng Lunch and Learn (May 8)**: Done. Eric presented AI coding showcase. Recurring series **canceled** (May 8 cancellation email from Piotr Kala — whole recurring Eng L&L removed from calendar).
- **Chris #2099 deal WS (+914, May 8)**: Largest PR of the week — WebSocket API for options deal entry. Part of the options booking flow (instrument puller + deal entry + deal WS all from Chris this week). Worth reviewing.
- **Billing Analytics Dashboard Demo**: June 1, 2026 8am CDT — Lily Chen invited Eric. Context: trade-level fee dashboard for financial oversight and reconciliation. Probably just an FYI / demo attendance request.
- **Project planning drafts**: `drafts/project_otc_options.md`, `project_kalshi.md`, `project_reconciliation.md` and others created May 6 for Eric's Notion project hub.
...
| P2.5 | OTC Options - HT Live | **GREEN** — Talgat started dev Apr 29; test trade May 1 dev | **May 15, 2026** (target reset Apr 30 in tracker; was May 30) |
...
### Pulse Tech Roadmap (from Drive, Apr 29)

Eric's roadmap items (marked `Thill *` = Eric owns):
1. **Perps booking** (P1) — Eric primary
2. **Persist OEMS orders** using CoinRoutes then self-built (P2)
3. **Fox River algos** (P3)
4. **Getting off Talos** — estimate delivered May 11: ~1 month to make Talos a database (passive dependency), then prioritize removal as a service. In progress organically.

Anton's roadmap items:
1. **Automate options RFQ** back-to-back w/ Paradigm+Deribit (P1) — Eric to speak to Raja + Sven about derivs post-trade risk
2. **Spreader** (P2) — Eric to speak to Chris D about Talos book perps + CoinRoutes to polaris (maker/taker)
3. **Options central risk book, legging strategy** (P3)
4. **Polaris pre-trade checks for PM Pro** (P4)

Anton/Thill shared:
- **Autoliquidation** (P2)
- **Perps funding rate** (P1)
- **Predictions perps streaming** (P2)
- **Multicasting/cross exchange** (P4)
- **Streaming** (P1)

Goal: a really good dealing/hedging system; clients can trade perps on anything; use spreader + Fox algos.
